...
1. **Phase 2 편집기 실사용 마무리**
   - [x] 결과 다운로드 포맷 확장 (`.json` 선택 저장)
   - [x] 리포트/비교 결과 뷰 필터(키워드 highlight) 도입
   - [x] 긴 결과 렌더 시 성능(스크롤/재렌더) 점검
2. **버전 API 품질 강화**
   - [x] report/csv 특수문자 edge-case 테스트 추가
   - [x] `report/top-fields` top_n 경계값 테스트 보강
3. **E2E 회귀 셋 고정**
   - [x] 에디터 경로 Playwright 스모크(버튼 클릭 + 결과 패널 변화) 1개 추가
   - [x] CI에서 최소 smoke subset을 별도 잡으로 분리
