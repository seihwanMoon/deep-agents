...
from ..services.secrets import encrypt_secret_value
...
async def create_secret(body: SecretIn, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    row = Secret(user_id=user.id, key_name=body.key_name, key_value=encrypt_secret_value(body.key_value), scope=body.scope)
    db.add(row)
    await db.commit()
...
async def update_secret(secret_id: int, body: SecretIn, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    row.key_value = encrypt_secret_value(body.key_value)
